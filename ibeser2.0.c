...
//----------------------------------------------------------------get MAC addr of current socket
	...
	{
		if( recv(connect_fd, recvbuf, 4096, 0) ==-1) 
		{
			printf("recv error.\n");
			exit(-16);
		}
		...
		if(recvbuf[0]=='1')//ClientHello
		{
			memcpy(&recvinfo, recvbuf, sizeof(recvinfo));
			//printf("TYPE is:%c\n", recvinfo.TYPE);
			//printf("cookie is:%d\n", recvinfo.cookie);
			memcpy(climac, recvinfo.macadd, sizeof(recvinfo.macadd));
			//printf("MAC is:%s\n", climac);
			if(recvinfo.cookie==-1)//No cookie, return HelloVerify
			{
				info2.TYPE='2';
				randcookie=rand();
				info2.cookie=randcookie;
				//printf("cookie is:%d\n", info2.cookie);
				//memcpy(sendbuf,&info2, sizeof(info2));
				if( send(connect_fd, (char*)&info2, sizeof(info2), 0) < 0)  
				{  
					printf("send error.\n");  
					exit(-10);  
				}
			...
			else//Has cookie, return ServerHello.
			{
				if(recvinfo.cookie!=randcookie)
				{
					printf("HelloVerify error!\n");
					return -1;
				}
				else//send ServerHello to client.
				{
					info3.TYPE='3';
					info3.cookie=randcookie;
					sprintf(info3.macadd, "%.2x:%.2x:%.2x:%.2x:%.2x:%.2x", mac_addr[0], mac_addr[1], mac_addr[2], mac_addr[3], mac_addr[4], mac_addr[5]);
					if( send(connect_fd, (char*)&info3, sizeof(info3), 0) < 0)  
					{  
						printf("send error\n");  
						exit(-10);  
					}
					printf("HelloVerify success!\n");
					break;// go out the while loop.
				}
			}
